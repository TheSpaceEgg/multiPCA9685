...
private:
    I2CDevice *i2c;  // i2c comms object
...
public:
     ...
     * 
     * @param bus_address_pairs A vector of bus and address pairs for the drivers.
     * @param frequency PWM frequency to be set for all drivers.
     */
    MultiPCA9685(const std::vector<std::pair<int, uint8_t>>& bus_address_pairs, uint8_t frequency = 50);
...
private:
    ...
    std::vector<std::pair<int, uint8_t>> busAddressPairs;
    uint8_t numDrivers;
    uint16_t DevFrequency; // default is 50. CHeck servo datasheet.
    const uint8_t outputsPerDriver = 16; // How many channels on each PCA9685 board need considering. If you only use first 12 on each board, set to 12 and channel 15 would be considered board 2
};
